...
for N in NList:
    ...
    for i in range(numTimeSteps):
        ...
        if (i < 2 or i % 2 == 0):
            ...
            lams = np.log(fs_solutions[-1] / fs_solutions[-2]) / (-1 * tau)
            lams = np.nan_to_num(lams)
            lams = np.array(lams)
            lams[lams > 5] = 5
            lams[lams < 0.01] = 0.01
            prev_fs_sol = sparse.csc_matrix(prev_fs_sol.toarray() * np.exp(- tau * lams))
...
plt.title('$H^1$-error at $t=%.2f$' % (numTimeSteps * tau), fontsize=16)
