'''
Fix required: Better looking plot.
'''
...
for k in kList:
    ...
    for i in range(numTimeSteps):
        print('Calculating correction at k = %d, i = %d' % (k, i))
...
plt.semilogy(kList, error, '--s')
plt.grid(True, which="both")
plt.xlabel('$k$', fontsize=30)
plt.title(r'The error $\|u^n-u^n_{\mathrm{ms}, k}\|_{H^1}$ at $T=%.1f$' % (numTimeSteps * tau), fontsize=32)
